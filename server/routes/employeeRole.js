...
router.put("/:ID", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Role updated successfully.", role: employeeRole });
  } catch (error) {
    console.error(error);
...
router.delete("/:ID", async (req, res) => {
  try {
    ...
    const employeesWithRole = await models.Employee.count({
      where: { role: ID },
    });
    ...
    if (employeesWithRole > 0) {
      return res.status(400).json({
        message:
          "Cannot delete role. There are employees assigned to this role.",
      });
    }
    ...
    if (error.message.includes("protected")) {
      return res
        .status(403)
        .json({ message: "This role is protected and cannot be deleted." });
    }
