...
router.post("/", async (req, res) => {
  try {
    const { clientNIC, totalPrice, employeeID, address, storeId } = req.body;

    if (!clientNIC || totalPrice == null) {
      return res
        .status(400)
    ...
    await models.sequelize.query(`
			SELECT setval(
				pg_get_serial_sequence('"ClientPurchases"', 'id'),
				(SELECT MAX(id) FROM "ClientPurchases")
			)
		`);
    const purchase = await models.ClientPurchase.create({
      clientNIC,
      total: totalPrice,
      employeeID: employeeID ?? "000000000",
      orderStatusID: 1,
      address: address ?? "",
      storeId: storeId ?? null,
    });

    return res.status(201).json({ id: purchase.id });
  } catch (error) {
...
router.delete("/:ID", async (req, res) => {
  ...
  try {
    ...
    const cartItems = await models.PurchaseCartEquipment.findAll({
      where: { clientPurchaseId: req.params.ID },
    });
    ...
    await transaction.rollback();
    console.error("Error deleting ClientPurchase and related data:", error);
    res.status(500).json({ error: error.message });
  }
});
