...
export default function DetailsModal({
	...
	return (
		<>
			<Modal
				show={show && !stateModal && !storesModal}
				onHide={() => {
					setShow(false);
					setIsEditing(false);
				}}
						...
						<Col xs={12} lg={6} className="d-flex flex-column mt-2">
							...
							>
								{isEditing ? (
									<>
										<Stack direction="vertical">
											<Form.Group controlId="equipmentStatus">
												...
												<Form.Control
													value={formData.equipmentStatus?.state || ""}
													readOnly
												...
												<Stack direction="horizontal" gap={2}>
													<Button
														className="rounded-pill"
														variant="secondary"
														disabled={!formData.equipmentStatus?.id}
														onClick={() =>
															setFormData((prev) => ({
																...prev,
									...
									<>
										<p>
											<span className="text-muted">Equipment Status:</span>{" "}
											{formData?.equipmentStatus?.state || "—"}
										</p>
			...
			<ComponentSelectorModalForm
				...
				onSelect={(selected) =>
					setFormData((prev) => ({
						...prev,
						equipmentStatus: { id: selected.id, state: selected.state },
					}))
				}
			/>
		</>
