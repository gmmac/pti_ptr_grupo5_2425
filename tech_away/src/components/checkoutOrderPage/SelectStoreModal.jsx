...
export default function SelectStoreModal({
	...
	useEffect(() => {
		...
		(async () => {
			try {
				...
				setFullList(rows);
				setList(rows);
				setSelectedRow(selectedStore || null);
			} catch (err) {
				console.error(err);
				setFullList([]);
				setList([]);
	...
	const handleRowClick = (row) => {
		...
		const isAlreadySelected = selectedRow && getRowKey(selectedRow) === rowKey;
		setSelectedRow(isAlreadySelected ? null : row);
	};
				...
				{list.length > 0 ? (
					<Table striped bordered hover responsive>
						...
						<tbody>
							{list.map((row) => {
								const rowKey = getRowKey(row);
								const isSel = selectedRow && getRowKey(selectedRow) === rowKey;
								return (
									<tr
										key={rowKey}
